...
회의를 녹음하기만 하면 Junmit이 말한 내용을 글로 옮기고(전사), 누가 말했는지 구분하고(화자분리), **우리 팀 양식에 맞춘** 회의록 초안까지 만들어 줍니다. 이미 쓰고 있는 Claude·ChatGPT·Google AI 구독으로 동작해 **추가 비용이 없고**, 구독이 없거나 회의 내용을 **전부 오프라인으로 처리**하고 싶다면 무료 로컬 AI로도 쓸 수 있습니다.
...
- **구독이 없다면 무료 로컬 AI로.** 로컬 AI(Gemma 4)를 선택하면 회의록 작성까지 전부 내 Mac 안에서 처리돼, 회의 내용이 밖으로 전혀 나가지 않습니다. 대신 기능과 품질에 제약이 있습니다([어떤 AI를 고를까요](#어떤-ai를-고를까요) 참고).
...
- **우리 팀 방식대로.** 발표/세미나·일반 회의·리뷰·회고·1on1 유형을 기본 제공하고, 팀에 맞춘 유형을 AI로 만들거나 직접 다듬을 수 있습니다. 자동 판단 모드는 회의 내용을 보고 유형을 알아서 고릅니다.
...
- **기능 범위.** 회의록 작성까지만 담당합니다. 전사본 교정(전사 오타 다듬기)과 작성 후 "AI에게 추가 요청"은 구독 AI에서만 지원됩니다.
...
1. **회의 선택.** 시작 화면에서 캘린더 일정을 고르거나 직접 입력합니다. 제목, 참석자, 회의 유형(발표/세미나·일반 회의·리뷰·회고·1on1, 또는 자동 판단)도 여기서 정합니다.
