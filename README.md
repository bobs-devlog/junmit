...
https://github.com/user-attachments/assets/8ada8cc1-4fa4-48db-91e9-f64f16bd737f
...
- **회의 유형에 맞춰 씁니다.** 발표/세미나·일반 회의·리뷰·회고·1on1 유형을 기본 제공하고, 어떤 유형인지 회의 내용을 보고 자동으로 판단하게 둘 수도 있습니다. 팀에 맞는 유형을 직접 만들어 쓸 수도 있습니다.
- **쓴 회의록을 스스로 검증합니다.** 초안을 만든 뒤에도 전사와 대조해 잘못 옮긴 수치·발언자·빠진 결정을 바로잡습니다. 구독 AI에서는 바뀐 부분이 "검증 N건"으로 표시되어 무엇이 고쳐졌는지 확인할 수 있습니다.
- **원격회의 상대방 음성까지.** 구글 밋·줌 등에서 내 목소리와 상대방 목소리를 함께 녹음합니다.
- **녹음 중 메모.** "지금 말하는 사람은 홍길동" 같은 화자 힌트나 자유 메모를 남기면 회의록에 반영됩니다.
- **회의록 다음 작업까지.** 작성이 끝난 뒤에도 회의록 수정, 결정사항만 추리기, 메신저·메일에 공유할 요약본 만들기 같은 후속 작업을 대화로 맡길 수 있습니다.
- **화자 사전.** 회의록에서 화자 이름을 한 번 확정하면 그 목소리를 기억해, 다음 회의부터 누가 말했는지 자동으로 채워 줍니다. 음성 정보는 내 Mac에만 저장되고 언제든 지울 수 있습니다.
...
5. **확인과 수정.** 회의록 탭에서 본문을 확인하고 복사하거나 `.md` 파일로 저장할 수 있습니다. 전사본 탭에서는 화자 이름을 확인하고 고칠 수 있으며, 고친 뒤 회의록을 다시 작성하게 할 수 있습니다.
6. **추가 요청 (선택).** 사이드바의 "AI에게 추가 요청"으로 회의록 수정, 결정사항만 추리기, 메신저·메일에 공유할 요약본 만들기 같은 후속 작업을 대화로 맡길 수 있습니다.

> 로컬 AI를 쓰는 경우 6번(추가 요청)은 제공되지 않습니다.

시작 화면 사이드바에서는 다음을 관리합니다.

- **회의 기록.** 지난 회의록을 다시 보고, 처리를 마치지 못한 회의는 남은 단계(전사·회의록 작성)를 이어서 마칠 수 있습니다. 제목은 물론 회의록·전사 내용으로도 검색할 수 있습니다.
...
AI가 앱 밖의 도구나 외부 서비스를 처음 쓰려 할 때 나오는 정상적인 확인 절차입니다. 키보드 방향키(↑/↓)로 허용(Yes)을 고르고 Enter를 누르면 그대로 진행됩니다.
