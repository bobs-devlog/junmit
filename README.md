...
회의를 녹음하면 Junmit이 알아서 말한 내용을 글로 옮기고(전사), 누가 말했는지 구분하고(화자분리), 회의록 초안까지 만들어 줍니다. 이미 쓰고 있는 Claude·ChatGPT·Google AI 구독으로 동작하므로 **추가 비용이 들지 않고**, 구독이 없다면 무료 로컬 AI로도 쓸 수 있습니다.
...
Apple Silicon Mac(M1 이후), macOS 14.4 이상에서 동작합니다. 처음 열 때 보안 확인이 한 번 필요합니다([설치하기](#설치하기) 참고).
...
- **이미 쓰는 AI 구독 그대로.** Claude(Pro·Max), ChatGPT(Plus·Pro), Google AI(Pro·Ultra) 구독 중 무엇이든 그대로 씁니다. 별도 결제나 복잡한 설정이 없습니다.
- **구독이 없다면 무료 로컬 AI로.** 로컬 AI(Gemma 4)를 선택하면 회의록 작성까지 전부 내 Mac 안에서 처리됩니다. 회의 내용이 한 글자도 밖으로 나가지 않는 가장 확실한 프라이버시 선택지입니다. 대신 기능과 품질에 제약이 있습니다([어떤 AI를 고를까요](#어떤-ai를-고를까요) 참고).
- **원격회의 상대방 음성까지.** 구글 밋·줌 등에서 내 목소리와 상대방 목소리를 함께 녹음합니다.
- **목소리는 내 Mac 밖으로 나가지 않습니다.** 전사와 화자분리는 기기 안에서 처리되고, 녹음 원본은 처리가 끝나면 자동 삭제됩니다. 회의록을 만들 때는 텍스트만 AI로 전달됩니다([내 데이터는 어디에 저장되나요](#내-데이터는-어디에-저장되나요) 참고).
- **우리 팀 방식대로.** 발표/세미나·일반 회의·리뷰 유형을 기본 제공하고, 팀에 맞춘 유형을 AI로 만들거나 직접 다듬을 수 있습니다. 자동 판단 모드는 회의 내용을 보고 유형을 알아서 고릅니다.
...
| [Antigravity](https://antigravity.google) | Google AI (Pro·Ultra 등) | Gemini 기반. Confluence 자동 등록은 추후 지원 예정입니다 |
...
- **기능 범위.** 회의록 작성까지만 담당합니다. 전사 정밀 교정(AI 다듬기), 작성 후 "AI에게 추가 요청", Confluence 등록은 구독 AI에서만 지원됩니다.
...
[최신 버전 받기](../../releases/latest)에서 `.dmg` 파일을 받아 연 뒤, `Junmit` 아이콘을 응용 프로그램(`Applications`) 폴더로 드래그합니다.

### 2. 처음 열 때 보안 확인 한 번

Junmit은 아직 Apple 코드 서명을 받지 않아, 처음 열면 "확인되지 않은 개발자입니다" 경고가 뜹니다. 아래 순서로 한 번만 허용해 주세요.

1. 경고 창에서 **확인**을 누릅니다.
2. **시스템 설정 → 개인정보 보호 및 보안**으로 이동합니다.
3. 아래쪽의 `"Junmit"을(를) 차단했습니다` 옆 "**그래도 열기**"를 누릅니다.
...
이 과정은 처음 한 번뿐입니다. 이후 새 버전이 나오면 앱 사이드바에 "↑ 업데이트 가능" 버튼이 뜨고, 설정 → 앱 업데이트에서 설치하면 됩니다(이때 macOS가 관리자 암호를 한 번 물을 수 있고, 보안 경고는 다시 뜨지 않습니다).

### 3. AI 도구 선택과 로그인

앱을 처음 열면 회의록을 작성할 AI 도구를 고르는 화면이 나옵니다. 쓰고 있는 구독에 맞는 카드를 선택하면 설치와 로그인이 앱 안에서 바로 진행됩니다. 화면의 안내에 따라 로그인만 하면 됩니다. 로컬 AI를 선택한 경우에는 로그인 없이 모델 내려받기만 진행됩니다.

> 평소 컴퓨터에서 이미 로그인돼 있어도 앱 안에서 한 번 더 로그인해야 합니다. Junmit은 개인 설정·기록과 섞이지 않도록 분리된 환경을 쓰기 때문입니다.

### 4. 필요한 파일 내려받기 (10~20분)

Setup 화면에서 "설치 시작"을 누르면 음성 인식에 필요한 파일들을 내려받습니다. 인터넷 속도에 따라 10~20분 정도 걸리니 잠시 다른 일을 해도 됩니다. 별도 가입이나 계정은 필요 없습니다.

### 5. 권한 허용

각 기능을 처음 쓸 때 macOS가 권한을 물어봅니다. 허용해 주세요.
...
**설치는 여기까지입니다.** 아래 두 가지는 필요할 때 연결하면 됩니다.

### (선택) 캘린더 연동

회의 일정을 자동으로 불러오려면 일정이 macOS 캘린더(Apple 캘린더)에 있어야 합니다. Google·Microsoft 등 외부 캘린더를 쓴다면 시스템 설정 → 인터넷 계정에서 계정을 추가하고 캘린더 동기화를 켜 주세요. 연동하지 않고 회의 제목과 참석자를 직접 입력해도 똑같이 쓸 수 있습니다.

### (선택) Confluence 발행

회의록을 Confluence에 등록하려면 Atlassian 계정 연결이 한 번 필요합니다. 처음 발행할 때 연결 승인 안내가 나오므로 미리 준비할 것은 없습니다([자주 묻는 질문](#자주-묻는-질문) 참고).

자동 등록(새 페이지 생성)은 Claude Code와 Codex에서 지원됩니다. Antigravity는 아직 자동 등록을 지원하지 않아(추후 지원 예정) 회의록을 복사해 직접 붙여넣는 방식으로 등록하고, 로컬 AI는 발행 기능이 없습니다.
...
1. **회의 선택.** 시작 화면에서 캘린더 일정을 고르거나 직접 입력합니다. 제목, 참석자, 회의 유형(발표/세미나·일반 회의·리뷰·자동 판단)도 여기서 정합니다.
...
6. **추가 요청 (선택).** 사이드바의 "AI에게 추가 요청"으로 회의록 수정과 요약은 물론, 결정사항으로 Jira 티켓 만들기, Slack·메일로 요약 공유, 후속 일정 등록 같은 후속 작업도 대화로 맡길 수 있습니다.
7. **Confluence 등록 (선택).** "Confluence 등록" 버튼을 누르면 발행됩니다. 처음 발행할 때만 Atlassian 연결 승인이 한 번 나옵니다.

> 로컬 AI를 쓰는 경우 6번(추가 요청)과 7번(Confluence 등록)은 제공되지 않습니다. Antigravity는 7번에서 자동 등록 대신 회의록 복사(직접 붙여넣기)를 안내합니다.
...
정상입니다(코드 서명 없음). [설치하기 2번](#2-처음-열-때-보안-확인-한-번)의 순서대로 한 번만 허용해 주세요.
...
외부 서비스를 처음 연결할 때 나오는 정상적인 확인 절차입니다. 키보드 방향키(↑/↓)로 허용(Yes)을 고르고 Enter를 누르면 그대로 진행됩니다. 주로 이런 때 나옵니다.

- Confluence에 처음 발행할 때(Atlassian 계정 연결 승인)
- 추가 요청으로 Gmail·Slack·캘린더·Notion 등 외부 서비스를 처음 연결할 때
...
- Confluence에 발행하면 **회의록**이 Atlassian으로 전달됩니다.
...
이 아래는 소스에서 빌드하거나 기여하려는 사람을 위한 내용입니다. 앱을 쓰기만 한다면 위 [설치하기](#설치하기)로 충분합니다.

```
녹음 → 전사 (whisper.cpp) → 화자분리 (pyannote.audio) → 회의록 (Claude Code / Codex / Antigravity / 로컬 Gemma·mlx) → Confluence
```
