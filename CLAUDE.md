...
> **실측 기록 (2026-07-11~12, 재논의 방지):** 전사 텍스트 교정은 회의록 품질에 기여하지 않음(5세션 A/B — 오타 유입 0·내용 정확도 무관, 가치는 전사본 열람·화자 매칭). 화자 매핑은 대화형 회의 품질에 실질 기여(매핑 없는 작성은 귀속 왜곡). 교정 background 분리를 구현·검증까지 했으나 **단순한 선형 서사(유지보수·사용자 이해) 우선으로 동기 회귀** — 비동기 재제안은 그 비용을 뒤집는 새 근거가 있을 때만. 교정 속도 검토(2026-07-27)의 결론: text-correction effort 상한을 명시 고정(claude frontmatter `effort: high`, codex는 gen-agent-skills.sh가 파생 생성하는 커스텀 에이전트 TOML) — 단 **앱은 격리 설정(CLAUDE_CONFIG_DIR·CODEX_HOME)으로 스폰해 원래 claude high·codex 하위 medium으로 돌고 있어 앱 경로엔 no-op**(시간 -48%/-38% 실측은 개인 CLI 설정(xhigh) 환경 기준 — 개인 설정 실측을 앱 효과로 읽지 말 것). 지침 스코프 축소·sonnet 전환은 기각, 재조정은 새 실측으로만.
