...
- `resources/` — 앱 동봉 자산 (release 시 `.app/Contents/Resources/`로 복사, dev에선 PTY cwd가 여기). IDE Claude Code의 워크스페이스 root와 분리하기 위한 의도적 배치
  - `resources/lib/` — 앱이 런타임에 호출하는 스크립트
    ...
    - `local_meeting.py`, `local_rules.md` — 로컬 LLM 회의록 백엔드 (active_cli=`mlx`, AI 구독 없는 사용자용). Gemma 4 12B(Apache 2.0, mlx-lm — 2026-08 mlx-vlm에서 이전: 기존 gemma4_unified 가중치를 우회 2건(모델 별칭 + vision_embedder 스킵)으로 그대로 로드, 같은 시드 생성 바이트 동일 실측. `enable_thinking=False` 명시 필수) 2종 — 표준(순수 4bit 6.8GB, 16GB Mac)/고품질(혼합 정밀도 11GB, 24GB+), 선택은 `~/Library/Application Support/app.junmit/local_model` 파일(Rust `cmd_set_local_model`이 기록, install.sh·python이 읽음). 파이프라인: 유형 자동 분류(auto 시) → 화자 매핑 준비(기존 매핑 보존 + 녹음 힌트 결정론 — 힌트는 미확정 화자 사전 프리필을 덮을 수 있음. LLM 제안은 실측 채택 0으로 제거, 팝오버 "AI 힌트"의 생산자는 원래 에이전트 CLI 경로뿐이었으나 화자 사전 프리필 reason이 전 경로 공통 소스로 추가됨) → 초안(교정본 우선, 9k 토큰 초과 시 map-reduce, 사용자 메모 앵커) → 자기검증(전사 대조로 시제·상태·누락 교정) → 결정론 후처리(헤더 주입·라벨/추측 병기 정리). 에이전트 아님 — MCP·assist 불가라 프론트가 게이팅(`cliHasAgent`). **실행은 PTY가 아니라 Rust `cmd_run_local_meeting`**(전사·화자분리와 같은 일반 서브프로세스 — stdout을 `local:output` 이벤트로 스트리밍해 진행 패널(LocalProgressPanel)에 표시. 단계 체크리스트는 stdout의 `@@progress` 마커(단계 계획 전체 + 현재 단계 자가완결, tty 직접 실행 시엔 한국어 줄)가 구동하고 시각 골격은 headless 패널과 공용 셸 `ProgressPanel` 공유, 완료/실패는 신호 파일 → app:signal). 모델 선택(Gemma 4 12B 2종)은 2026-07 후보 비교 실측으로 확정 — 재검토는 새 실측으로만
  ...
  - `resources/vocabulary.json` — 용어 사전 **시드**. 첫 실행 시 `~/Library/Application Support/app.junmit/vocabulary.json`으로 복사된다 (사용자 영역, 단일 진실 원천). 앱의 "용어 사전" 화면에서 등록/수정/삭제하며 후보정 교정·회의록 작성이 읽는다 (whisper `--prompt`에도 실리지만 max-context 0에서는 무효 — 근거는 transcribe.sh 주석). `{ "terms": [...] }` 객체 래퍼 (추후 형제 필드 확장 여지)
...
| `speaker_mapping.json` | 화자 이름 매핑 (사용자 수정 가능, 단일 진실 원천). 화자분리 단계가 화자 사전(전역 음성 DB)과 매칭되면 추정(confirmed 없음)으로 프리필 — reason 접두 `지난 회의 목소리 일치`(이름 확정)/`지난 회의 목소리 후보`(이름 없는 힌트 — 후보 근접 또는 참석자 명단 밖)이 판별 마커이며 speaker-mapping sub-agent·SKILL.md 다듬기 OFF·local_meeting.py가 공유(증거 우선순위: 사용자 화자 힌트 > 직접 텍스트 증거 > 화자 사전 프리필 > 간접 cue) |
| `speaker_embeddings.json` | 화자별 대표 음성 임베딩 (화자분리 단계 기록 — recording.wav 삭제 전 유일 시점. 사용자가 화자를 확정하면 앱이 전역 `speaker_db.json`에 등록하는 재료. 화자 자동 인식 OFF 세션·옛 세션엔 없음 → 등록 자연 skip) |
...
6. 회의록 자기검증 (`notes-verification` sub-agent 2개 병렬 — 귀속·수치 / 블록 누락 분담, 전사 대조 보고 → 메인 적용 → `notes_verification_report.json` + `app_phase_step_done verify` **항상** 전송). 공개 후 사후 다듬기 — 검증 동안 앱은 회의록 편집·유형 변경·AI 추가 요청을 잠그고 **헤더·사이드바를 "검증 중"으로 표시**(완료 띠·회의록 step ✓는 verify 신호 후에만 — 미리 "완료"로 보이면 진행 중 인지 불가. verify 신호로 해제, 신호 유실 시 10분 타임아웃·PTY 종료가 회수) 완료 시 **회의록 탭만 재로드**(전사본 탭 불간섭 — 공개 직후의 화자 매핑 작업 보존), 바뀐 내역은 "검증 N건" 칩으로 노출. `meeting.json.notes_verification: false`(UI "회의록 검증" 토글, 기본 ON)면 생략하며 앱도 잠그지 않음(로컬 파이프라인의 내장 자기검증과 무관, 에이전트 경로 전용). 재작성 시 이전 영수증은 앱이 제거(Rust `backup_meeting_notes`·`cmd_clear_verification_report`). **끝나면 macOS 알림(검증 ON/OFF 공통, 항상 마지막)** — 알림으로 복귀한 사용자가 검증까지 끝난 완성본을 보게

> **실측 기록 (2026-07-11~12 / 08-09 / 08-14, 재논의 방지):** 전사 텍스트 교정은 회의록 품질에 기여하지 않는다(5세션 A/B: 오타 유입 0·내용 정확도 무관. 가치는 전사본 열람·화자 매칭). 화자 매핑은 대화형 회의 품질에 실질 기여(매핑 없는 작성은 귀속 왜곡). 교정 background 분리는 구현·검증까지 갔으나 선형 서사(유지보수·사용자 이해) 우선으로 동기 회귀했다(비동기 재제안은 그 비용을 뒤집는 새 근거가 있을 때만). **text-correction은 2026-08-09 재설계로 종결**(07-27 "스코프 축소 기각"은 품질 유지 전제였고, 사용자가 기준을 "명백한 오타만·전사본 열람용"으로 명시 하향해 전제 소멸): 확정 = 명백 매칭 화이트리스트(vocabulary·attendees·음절, "등장하는 모든 줄 대조") + 문맥 교정은 "찾아 나서지 않는 재량"(눈에 띈 고확신만 estimated·reason) + 청크 300줄 + frontmatter `effort: medium`. 923줄 픽스처 620초→133~156초(-76%), vocabulary recall 80%대·참석자 이름 93%. 기각 = effort low(recall 35% 붕괴), 청크 500(Read 상한 잘림 위험), 같은 줄 한정 동음이의 복원(실측 2회 0건), sonnet(보류). **함정 경고: text-correction.md 도입부의 배타 문구("그 외는 원문 유지가 정답")가 시간 예산의 앵커다.** 중립 문구로 완화한 실측에서 시간 2배·이름 recall 회귀가 즉시 재현됐다(재량 절의 "찾아 나서지 마라" 단독으론 못 막음). 이 파일의 지침 문구 수정은 반드시 픽스처 1 run 검증을 동반할 것. **speaker-label-correction은 2026-08-14 종결**: 확정 = frontmatter `effort: medium`(격리 281초→225·182초, 재할당 코어 무손실 2회). 기각 = 청크 100→300(319초 역효과: 턴테이킹 분석은 청크 확대가 턴당 추론을 늘려 text-correction 처방이 이식되지 않음). "한 라벨 두 화자" 병합 블록 탐지는 effort와 무관하게 비재현(격리 0/2)이라 이 에이전트에 기대하지 말 것: 신뢰 가능한 병합 탐지는 세그먼트 임베딩 분산 기반 결정론 전환(백로그, pyannote 단 산출 필요)이 유일 경로. 공통 사실: frontmatter effort는 앱 격리 홈의 세션 기본(high)보다 우선한다(claude 2.1.223 sub-agent 로그 검증). 재조정은 새 실측으로만.

### 2. 사용자 검토
- 회의록 탭에서 본문 검토 + 화자 매핑 검증. 필요 시 탭 툴바 "복사"(리치텍스트 클립보드) 또는 "파일로 저장"(.md, 이름 치환본 — 디스크 원본과 달리 SPEAKER_XX가 실명으로 치환됨).
- 회의 기록 화면 상단 검색: 제목은 즉시, 본문(참석자·회의록·전사)은 입력 멈춤 후 Rust 검색(`cmd_search_sessions`). 파일은 SPEAKER_XX 원본이라 화자 이름 질의는 참석자(meeting.json) 매칭이 담당.
...
- **캘린더 권한 자동 재요청**: 비공증 배포는 업데이트마다 서명이 바뀌어 TCC가 리셋된다(권한 not_determined 복귀). 연동 이력을 `~/Library/Application Support/app.junmit/calendar_connected`("1"/"0")로 기억해, 홈(MeetingSelector) 마운트가 not_determined + 이력 "1"이면 버튼 없이 자동으로 권한을 재요청한다(마이크와 같은 경험. 거부하면 "0"으로 꺼져 버튼 경로만 남음). 이력은 권한 상태 조회(`cmd_check_calendar_permission`)가 동기화(authorized→"1", denied/restricted→"0")하되 **dev 빌드는 동기화 제외**(dev는 캘린더 권한이 늘 거부로 보여 release와 공유하는 플래그를 "0"으로 밟게 됨. README "dev 빌드 캘린더 미동작" 참조). 파일 부재 시 과거 세션 `source: "calendar"` 존재 여부로 1회 부트스트랩(기능 도입 전 연동자 구제).
- 참석자 이름은 자유 형식(영문·한글·풀네임 가능). 캘린더 참석자는 **`EKParticipant.url`에서 뽑은 식별자를 안정 키**로 삼아 표시 이름을 해결한다: ① 사용자 매핑 캐시(`~/Library/Application Support/app.junmit/attendee_names.json`, `{ "식별자": "name" }`) → ② EKParticipant.name(이메일꼴이 아니면) → ③ 이메일 local-part 휴리스틱(`. _ -` 분리·capitalize, 예: `bobs.kim@x.com` → `Bobs`) → ④ 빈 이름(사용자가 채우도록 추정 행으로 노출). MeetingSelector에서 이름을 인라인 편집하면 그 식별자에 귀속돼 다음 회의부터 자동 적용된다(EventKit displayName은 Google Workspace에서 이메일로 fallback되므로 신뢰하지 않음). **키가 늘 이메일인 건 아니다** — 참석자 값(CAL-ADDRESS)은 임의의 URI이고 이메일을 가리킬 때만 `mailto:`여야 하며(RFC 5545 §3.3.3), 캘린더 사용자 주소가 마땅치 않은 참석자(본인 계정 등)는 서버가 principal 리소스 URI로 대체할 수 있다(RFC 6638 §2.4.1). 2026-08 실측에서도 본인 항목이 `/a…` 경로로 내려왔고, 같은 형태가 rdar://35611698에 보고돼 있다. 그 값도 캐시 키로는 유효하나 이메일로 다뤄선 안 된다. 그래서 참석자 DTO가 `id`(항상 = 캐시 키)와 `email`(mailto일 때만)로 분리돼 있고, **판정은 URL 스킴을 볼 수 있는 Swift(`isEmailParticipant`)에서만** 한다 — 프론트가 문자열로 되짚으면 `@`가 섞인 principal URL을 이메일로 오인한다. `id`는 "mailto만 벗긴 원문"이어야 기존 캐시 키가 계속 맞는다(정의를 바꾸면 사용자가 저장한 이름이 전부 미아). EventKit은 이메일을 공개 API로 노출하지 않아 이 URL이 유일한 출처이며, `isCurrentUser`는 주최자에게만 붙어 본인 판별에 못 쓴다(rdar://15396225). 이메일이 없는 참석자는 이름을 ②나 사용자 입력에 의존한다
...
- **화자 사전(화자 DB)**: `~/Library/Application Support/app.junmit/speaker_db.json` — 사용자가 화자 매핑을 확정한 목소리를 인물별 임베딩 샘플로 축적(인물당 최대 8개, `(session_id, speaker)` 키 upsert라 이름 변경 = 샘플 이동. 인물 이름 매칭은 등록·매칭 모두 **대소문자 무시** — "bobs"/"Bobs" 변형이 두 인물로 갈라지면 margin 경합으로 프리필이 조용히 보류되는 함정). 화자분리(pyannote_diarize.py)가 읽어 다음 회의 이름을 프리필하고(인물별 최대 샘플 코사인, 임계 0.75·1↔2위 margin 0.05. **이름 대신 후보 힌트로 강등하는 두 경우** = margin 미달·참석자 명단 밖 인물 — 명단 대조는 토큰 단위 느슨 일치("Wayne"↔"Wayne Kim")라 대조 오판은 힌트 강등으로만 떨어짐, 명단 미입력 회의는 강등 없음), 확정 시 프론트(`src/utils/speakerDb.ts` `enrollSpeakerVoice`)가 등록. `speaker_profile_off` 센티넬 존재 = 화자 자동 인식 OFF(수집·매칭 전체 중단, keep_recording 관례). 관리 화면은 사이드바 "화자 사전"(목록·삭제·토글). 임계·margin은 2026-07 실측 확정(동일 참석자 2세션 교차: 동일인 0.85~0.93, 타인 최대 ~0.50) — 조정은 새 실측으로만.
