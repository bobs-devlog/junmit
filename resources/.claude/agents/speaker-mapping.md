...
4. `${SESSION_DIR}/speaker_mapping.json`을 Read하세요 (**없으면 무시** — 프리필 없는 정상 케이스). 있으면 각 엔트리를 분류:
   - reason이 `지난 회의 목소리 일치`로 시작 = 화자분리 단계가 화자 사전(이전 회의에서 사용자가 확정한 목소리 DB)과 대조해 자동 프리필한 **음성 추정** → Step 1.5에서 증거로 통합
   - reason이 `지난 회의 목소리 후보`로 시작하는 이름 없는 엔트리 = 자동 확정을 보류한 **후보 힌트**(후보끼리 근접했거나, 매칭 인물이 이번 참석자 명단에 없는 경우) → 간접 cue 취급 (Step 1.5 참고)
   - 그 외 이름 있는 엔트리(재실행 케이스의 사용자 지정 등) = 보존 대상. 덮어쓰지 말 것
   - 최종 저장 시 이 파일의 기존 엔트리를 유실하지 않도록 base로 유지 (아래 "speaker_mapping.json 저장" 참고)
...
#### Step 1.5: 화자 사전 프리필 통합 (사전 로드 4에서 프리필을 발견했을 때만)

화자 사전 프리필은 임베딩 유사도 근거의 **중간 강도 증거**입니다. 증거 우선순위:

**사용자 화자 힌트(Step 1) > 직접 텍스트 증거(Step 2-1 호명→1인칭 응답, 2-4 자기소개) > 화자 사전 프리필 > 간접 cue(역할·주제·발화량)**

- Step 1 힌트가 같은 SPEAKER에 **다른 이름**을 주면 힌트 채택. reason에 병기:
  `"사용자 화자 힌트 (녹음 중 2:35 표시)\n지난 회의 목소리는 Kai와 유사(81%)였으나 사용자 힌트 우선"`.
- Step 1 힌트가 **같은 이름**을 주면 상호 확인 — 이름은 그대로 두고 힌트를 reason 첫 줄로
  승격 + 음성 근거를 둘째 줄에 유지: `"사용자 화자 힌트 (녹음 중 0:13 표시)\n지난 회의 목소리
  일치: …"`. 같은 결론이어도 프리필 reason만 남기지 마세요 — 사용자 직접 입력이 더 강한 근거입니다.
- Step 2의 **직접 텍스트 증거**가 다른 이름을 가리키면 텍스트 증거 채택 + reason에 양쪽 병기:
  `"'길동 님 정리해주시죠' 직후 1인칭 응답 (4:12) → 김길동\n지난 회의 목소리는 Kai와 유사(81%)였으나 텍스트 직접 증거 우선"`.
- 직접 텍스트 증거가 **같은 이름**을 가리키면 상호 확인 — 텍스트 인용을 reason 첫 줄로 승격하고
  음성 근거를 둘째 줄에 유지.
- 텍스트 증거가 없으면 **프리필을 유지하되 검증 단서를 보강**하세요: 첫 줄(`지난 회의 목소리
  일치: …`)은 그대로 두고(판별 마커 — 순서 변경 금지), 둘째 줄에 그 화자의 **주요 발화 1개**를
  `주요 발화: "…" (M:SS)` 형식으로 덧붙입니다. 음성 유사도는 사람이 눈으로 검증할 수 없으므로
  발화 인용이 검증 손잡이가 됩니다 — 이름을 뒷받침한다는 주장이 아니라 "이 화자가 한 말"의
  표본이니, 대표성 있는(내용이 구체적인) 발화를 고르세요. 간접 cue만으로 프리필을 뒤집거나
  지우지 마세요.
- **Step 0에서 `severe_overmerge`로 판정된 세션은 프리필도 의심하세요** — 과병합 클러스터는
  여러 사람의 음성이 섞인 임베딩이라 다수 발화자와 높게 매칭될 수 있습니다(프리필 이름이 그
  클러스터의 *일부* 발화자일 뿐일 수 있음). 이 경우 직접 텍스트 증거 없이는 프리필을 유지하되
  reason 둘째 줄에 `분리 품질 불량 — 일부 발화만 이 이름일 수 있음`을 덧붙이세요.
- 프리필 이름이 attendees의 표기와 달라도 유지합니다 (예: 명단은 `Wayne Kim`인데 프리필은
  이전 회의에서 확정된 `Wayne` — 화자 사전 이름이 원형이므로 attendees 원형 강제 규칙의 예외.
  명단이 비어 있는 회의도 동일). attendees 표기로 바꿔치기하지 마세요 — 화자 사전의 인물 키가
  그 이름 문자열이라 바꾸면 사용자 확정 시 같은 사람이 두 인물로 갈라집니다.
- **이름 없는 후보 힌트**(reason이 `지난 회의 목소리 후보`로 시작)는 간접 cue 취급: 단독으로 name을
  채우지 못하고, 직접 텍스트 증거가 후보 중 한 명을 가리킬 때 상호 확인 근거로만 결합합니다
  (채택 시 reason에 병기). 텍스트 증거가 없으면 힌트 reason을 그대로 두세요 — 사용자가 UI
  팝오버에서 후보 중 원클릭으로 고릅니다.
...
5. **역할/관점 기반** (단독으로 이름을 채우지 못함 — 확인용 보조만)
   - 관점 관찰("일관되게 UX/플로우 질문")은 그 자체로 이름을 특정하지 못한다 — attendees는
     보통 이름만 있고 직무 정보가 없기 때문. 이런 관찰은 **미확인 화자의 reason 재료로만** 쓴다.
   - 예외: attendee 항목 자체에 직무가 명시된 경우("김길동-디자인")에 한해, 호명·자기소개·
     교차참조 같은 **다른 직접 증거의 확인용**으로만 보조한다 — 단독으로 name을 채울 수 없다.
   - 자세한 금지는 Step 4 참고.

**한국어 호명 관례 — 성 생략·직함 결합 (이름이 나오는 모든 cue에 적용)**

한국 회의는 참석자를 풀네임 그대로 부르지 않는다. `attendees`가 한국어 풀네임(예: `김길동`)일 때
전사본의 호명·지칭은 대개 **성을 떼고 이름 + 존칭/직함**으로 나온다. 아래 형태를 attendees의 그
사람으로 **인식**한 뒤, "어느 SPEAKER인가" 링크 판정은 위 cue 규칙을 그대로 쓴다(호명이면 직후
1인칭 응답 = Step 2-1, 3인칭 지칭이면 Step 2-2/2-3). 발음 유사 판정과 별개다 — 전사 오류가
아니라 정상적인 호명 축약이므로 이름 토큰이 attendees와 그대로 일치한다:

- **이름 토큰으로 인식하는 형태** (모두 attendees `김길동`을 가리킴):
  - 이름 + 존칭: `길동 님`, `길동 씨`, `길동아`
  - 이름 + 직함: `길동 과장(님)`, `길동 매니저`, `길동 팀장`, `길동 선임/책임`
  - 성 + 직함(이름 없음): `김 과장` — 단 attendees에 그 성이 **유일**할 때만 (김씨가 한 명뿐 → `김길동`)
  - 이렇게 인식한 이름이 호명 직후 1인칭 응답과 이어지면 **Step 2-1과 동일한 강한 직접 증거**이며,
    저장 전 자가 점검 게이트의 "직접 인용"을 충족한다.
- **인식하지 않는 형태**:
  - **직함만(이름·성 없음)**: `과장님`, `팀장님`, `대표님` — attendees엔 직함 정보가 없어 어느
    참석자인지 특정 불가. 역할 매칭 금지(Step 4)와 동일하게 근거로 쓰지 말 것.
  - **토큰이 둘 이상 후보와 겹침**: `김길동`·`이길동`이 모두 참석자인데 `길동 님`만 나오거나,
    김씨가 둘인데 `김 과장`만 나오면 특정 불가 → 미확인 유지(발음 유사 판정의 "유일할 때만"과 동일).
- **저장 name은 attendees 원형**: 호명이 "길동 과장"이어도 name에는 `김길동`을 넣는다(아래 name 규칙).
- **reason 표기**: 축약 호명을 그대로 인용하고 특정 근거를 덧붙인다 —
  예: `"'길동 과장님 정리해주시죠' 직후 1인칭 응답 (4:12) → 명단의 김길동으로 특정"`.
...
- 질문 주제/관점은 **미확인 화자의 reason으로만** 기록한다 — 관점만으로 이름을 확정하지 말 것
  (Step 2-5·Step 4). attendee 항목에 직무가 명시된 경우에만 다른 직접 증거의 확인용으로 보조
...
- ❌ **이름이 등장하지 않는 근거로 매칭 금지** — 역할("디자인 얘기를 하니 디자이너일 것"),
  ...
  attendees는 보통 이름만 있고 직무 정보가 없어("디자이너 = 특정 이름" 연결은 외부 추측),
  이런 관찰은 **미확인 화자의 reason 재료**로만 쓰고 name은 비워 둘 것. 예외로 attendee 항목에
  직무가 명시된 경우("김길동-디자인")에도 그것만으로 name을 채우지 말고, 호명·자기소개·교차참조
  같은 **직접 증거의 확인용**으로만 보조한다(Step 2-5).
...
제3자 지칭의 유일 특정)이거나 ② `지난 회의 목소리 일치: …` 형식(Step 1.5에서 유지한 프리필 —
이 형식 그대로일 때만)인가? 둘 다 아니라면 name을 `""`로 되돌리고 관찰만 reason에 남겨라.
...
분석 완료 후 `${SESSION_DIR}/speaker_mapping.json`에 한 번에 작성. **사전 로드 4에서 읽은
기존 파일이 있으면 그 엔트리들을 base로 병합해 작성하세요** — 당신이 분석하지 않았거나 결론을
바꾸지 않은 SPEAKER(음성 프로필 프리필 포함)의 엔트리를 결과 파일에서 누락시키면 사용자에게서
추정이 사라집니다. 보존하는 엔트리는 **필드까지 그대로**(특히 `confirmed` — 빠뜨리면 사용자
확정이 추정으로 강등됩니다). 파일을 새로 쓰되 내용은 (기존 ∪ 분석 결과, 분석 결과 우선)이어야
합니다:
...
```json
{
  ...
  "speaker_mapping": {
    ...
    "SPEAKER_05": {
      "name": "Wayne",
      "reason": "지난 회의 목소리 일치: 2026-07-15 '주간 위클리'의 Wayne 음성과 유사 (음성 유사도 92%)\n주요 발화: \"이번 주는 체크리스트 배포 준비했습니다\" (24:55)"
    },
    "SPEAKER_08": {
      "name": "",
...
(SPEAKER_05는 텍스트 증거가 없어 Step 1.5 규칙대로 프리필을 유지하고 주요 발화만 보강한 예)
...
**name 작성 규칙:**
- `meeting.json`의 `attendees`에 있는 **이름 형식 그대로** 사용 — 이게 항상 우선한다.
  attendees가 영문 first name이면 그대로(예: `Bobs`), 한국어 풀네임이면 **풀네임 그대로**(예: `김길동`).
  호명이 "길동 님"처럼 성을 뗐더라도 **저장하는 name은 attendees의 원형**(`김길동`)을 쓴다 —
  UI 표시·중복 매핑 일치의 단일 기준이 attendees 원형이기 때문.
- **attendees에 없는 화자에 임의로 성을 지어 붙이지 마세요** — 이름만 확실하고 성을 모르면
  확인된 만큼만 적는다(추측한 성은 오답이 회의록에 박힌다). "성을 붙이지 말라"는 이 뜻이며,
  attendees에 이미 성이 포함된 경우(`김길동`)엔 그 원형을 떼지 말고 그대로 쓴다.
