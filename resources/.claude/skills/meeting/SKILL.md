...
| 4 | 마무리 | 마무리하는 중 → 검증 진입 시 "회의록을 검증하고 마무리하는 중" | 5단계 (공개 신호) + 6단계 (자기검증·app_refresh·알림 — 검증 ON일 때 activeForm 갱신으로만 표시, 별도 단계 아님) |
...
transcript_corrected.txt (교정 완성본) + speaker_mapping.json — 이 시점부터 앱이 화자 매핑 편집 허용
       ...
       │  ⑥ 자기검증 (6단계, 검증 ON) — notes-verification 2개 병렬 → 메인 적용 → app_refresh + report
       │     → macOS 알림 (검증 ON/OFF 공통, 마지막)
       ▼
...
- **3 → 4 → 5(공개) → 6(검증)단계는 첫 작성과 동일하게 모두 실행**. 5단계의 `app_phase_done`·6단계의 알림 신호 누락 X — 신호가 없으면 frontend가 review 화면 전환·알림 전송을 못 함.
...
0. **사전 로드** — `~/Library/Application Support/app.junmit/vocabulary.json`을 Read (공통 규칙 "음성 인식 오류 교정" 참고). 교정 OFF에선 전사 텍스트 교정 단계가 없으므로, 작성하면서 vocab·attendees·문맥으로 음성 오인식을 자체 교정해야 합니다 (교정 ON에서도 무해한 추가 안전망).
...
회의록을 공개한 뒤, 전사와 대조해 사후 검증하고 반영합니다. 공개된 초안이 검증으로 소폭 바뀔 수 있으나(사용자가 이미 볼 수 있음), 바뀐 내역은 "검증 N건" 칩으로 투명하게 노출되므로 문제없습니다.

**선행 분기**: `meeting.json`의 `notes_verification` 필드가 **`false`면 검증(1~4번)을 통째로 건너뛰고** 5번 알림만 보냅니다 — 사용자가 속도/토큰을 위해 검증을 끈 것 (UI 표시명 "회의록 검증", 기본 ON). `true` 또는 필드 없음이면 수행:
...
3. 두 보고를 받으면 **메인이 지적들을 `meeting-notes.md`에 직접 적용** (Edit — 보고의 원문/수정 쌍을 기계적으로 반영. 두 보고가 같은 문장을 다르게 고치라면 라인 근거가 더 구체적인 쪽 채택). 적용이 1건 이상이면 내역을 `$SESSION_DIR/notes_verification_report.json`에 Write — 앱이 회의록 탭에 "검증 N건" 칩으로 표시하는 원천:
   ```json
   {
     "applied": [
...
4. **3번에서 report 파일을 썼으면 화면 갱신 (필수)** — 이미 공개된 회의록이라 이 신호가 없으면 검증 반영분·매핑 의심 칩이 화면에 안 보입니다. **회의록 수정 1건 이상 또는 매핑 의심 기록(둘 중 하나라도 = report 파일을 쓴 경우) 모두 보냅니다** — 앱 화면(회의록 탭)은 phase_done 시점에 이미 마운트돼 있어, 이 신호 없이는 새로 쓴 report 파일을 다시 읽지 못해 "⚠ 화자 확인" 칩도 안 뜹니다. 적용 0건 + 매핑 의심도 없어 파일을 안 만든 경우에만 생략:
   ```bash
   bash -c 'source "$APP_DIR/lib/signal.sh" && app_refresh'
   ```
   그 뒤 검증 결과 한 줄 출력 (0건이면 생략): `📋 검증 {V}건 반영`. 터미널에 diff는 출력하지 않습니다.
   - **sub-agent가 실패하거나 비정상 종료해도 중단하지 않습니다** — 살아있는 쪽 보고만 적용하고(둘 다 실패면 초안 그대로) 5번으로 진행 (검증은 안전망이지 관문이 아님).
5. macOS 알림 전송 (사용자가 다른 앱에 있을 수 있으므로 — 검증 ON/OFF 공통, 항상 마지막에):
   ```bash
