...
**TodoWrite로 진행 상황 표시** — `SESSION_DIR`을 잡으면 **맨 처음 도구 호출로** 다음 4개 항목 todos를 초기화하세요(`회의 정보 확인`을 `in_progress`로 시작). **도구 목록에 `TodoWrite`가 없으면** 이 문단과 이후 모든 TodoWrite 지시(단계 갱신·`completed` 마킹 포함)를 생략하는 대신, **단계 전환마다 아래 표의 activeForm 문구를 `⏳ {activeForm}…` 한 줄로 출력**해 지금 무엇을 하는 중인지 알리세요 (검증 진입 시 "회의록을 검증하고 마무리하는 중" 전환 포함). 대체 도구 탐색·부재 보고는 하지 않습니다 — [.claude/CLAUDE.md](../../CLAUDE.md) 폴백 규칙: 한 단계 한 줄, 도구 호출 중계 금지. 검증은 별도 항목이 아니라 "마무리" 안에 포함됩니다 — 6단계(검증) 진입 시 목록 구성은 그대로 두고 **"마무리" 항목의 activeForm만 "회의록을 검증하고 마무리하는 중"으로 갱신**해 진행을 보입니다 (검증 OFF면 갱신 없이 마무리). **`ls`·`cat`·`Read` 등 어떤 파일 확인·읽기보다 먼저** 박스를 띄워, 사용자가 보는 첫 화면이 셸 명령이 아니라 "회의 정보 확인하는 중" 박스가 되게 합니다 — 박스보다 파일 읽기가 먼저 보이면 멈춘 듯·개발자 화면처럼 느껴집니다. **선행 확인·`meeting.json` 읽기는 박스를 띄운 *뒤* "회의 정보 확인" 단계 안에서** 합니다 (그래야 파일 읽기 흔적이 "회의 정보 확인하는 중" 항목 아래 자연스럽게 놓임):
...
|---|---|---|---|
| 1 | 회의 정보 확인 | 회의 정보를 확인하는 중 | 선행 확인(전사 완료·재작성 여부) + `meeting.json` 읽기(AI 다듬기 여부·유형) |
...
| 4 | 마무리 | 마무리하는 중 → 검증 진입 시 "회의록을 검증하고 마무리하는 중" | 5단계 (공개 신호) + 6단계 (자기검증·verify 신호·알림 — 검증 ON일 때 activeForm 갱신으로만 표시, 별도 단계 아님) |

**"회의 유형 결정"을 별도 todo로 두지 않는다** — type이 `auto`면 유형 분류가 "AI 다듬기"에서 병렬로 끝나고, 명시 유형이면 결정할 게 없다. 3단계가 하는 일(결정 적용 + 가이드 로드)은 거의 즉시라 "회의록 작성" 항목에 흡수한다. (1단계를 skip한 재작성·재시도에서 유형 판단을 인라인으로 할 때도 "회의록 작성" 항목 안에서 처리). **"회의 정보 확인"은 이와 별개** — 전사 완료·재작성 여부·AI 다듬기 여부 같은 *선행 판단*이라 작업 시작 직후 실제로 수행하는 단계이고, 박스를 첫 출력으로 만들기 위해 1번 항목으로 둔다.
...
- **AI 다듬기 OFF** (`meeting.json`의 `ai_polish: false`, [1단계의 "AI 다듬기 여부 확인"](#1단계-화자-라벨-교정--화자-매핑--전사-텍스트-교정-병렬-sub-agent) 참고) — 1단계 sub-agent를 하나도 띄우지 않으므로, 화자 힌트 반영·`correct` 신호까지 마치면 "AI 다듬기"를 즉시 `completed`로 마킹하고 "회의록 작성"으로 진행
- **1단계 sub-agent 병렬 spawn(AI 다듬기 ON(기본) 3개 + type이 auto면 유형 분류 +1개 / OFF 0개)은 "AI 다듬기" todo 한 항목으로 묶음** — 메인이 모든 sub-agent 결과를 받은 뒤 자체 마킹만 업데이트
...
2. **SPEAKER_XX sentinel** — 회의록의 **발화 주체 표기**(발언 헤더, 의견 출처 라벨)는 `SPEAKER_XX` 라벨을 유지하고, 앱이 표시 시점에만 실제 이름으로 치환. 발화 주체 표기에 실제 이름을 박지 않음. 단 **산문 안에 직접 언급된 인물 이름은 자연어 그대로** (sentinel은 자동 화자분리의 불확실성에만 적용, 발화 텍스트에 등장한 이름은 확실한 정보). **그룹핑/메타 표기**(sub-section 헤더, 발표자 표기)와 **Action Items의 `@assignee`**는 별도 분기 — 아래 표 참고
...
| `transcript_text_edits.json` | LLM (1단계) | sidecar, 앱 UI | 텍스트 교정 명세. sidecar가 corrected.txt에 적용한 뒤, 이 JSON을 실패 항목 제외하고 재작성 (`time`은 sidecar가 라인 헤더에서 주입). AI 다듬기 OFF(`ai_polish: false`)면 생성되지 않음 |
| `transcript_speaker_edits.json` | LLM (1단계) | sidecar, 앱 UI | 화자 라벨 재할당 명세. sidecar가 corrected.txt에 적용한 뒤, 이 JSON을 실패 항목 제외하고 재작성. AI 다듬기 OFF면 생성되지 않음 |
| `speaker_mapping.json` | LLM, 사용자 (앱 UI) | LLM, 앱 UI | 화자 → 이름 매핑의 **단일 진실 원천**. AI 다듬기 OFF면 화자 힌트(notes.json)가 있을 때만 메인이 최소로 작성 |
| `meeting-notes.md` | LLM (이 스킬), 사용자 (앱 UI) | 앱 UI, 사용자 | 회의록 본문(유일한 본문 파일). SPEAKER_XX 라벨 유지 (앱이 표시 시점에 치환) |
| `meeting.json` | 앱 (녹음 단계), 사용자 (앱 UI), LLM (type 갱신만 — [notes-rules.md](notes-rules.md) "type 갱신 절차") | LLM | 회의 메타데이터 단일 진실 원천. 필드: `title`, `date`, `time?`, `type`, `attendees`, `agenda`, `source`, `ai_polish`(AI 다듬기 토글 — 1단계 진행 여부 분기), `notes_verification`(회의록 검증 토글 — 6단계 분기) |
...
| `notes_verification_report.json` | LLM (6단계 메인 — 검증 적용 시) | 앱 UI | 검증에서 반영된 수정 내역 + 매핑 의심 경고. 적용 0건·검증 OFF면 없음. 회의록 탭 "검증 N건" 칩의 원천 |
...
transcript.txt (whisper 산출물, 변경 금지)
       │
       │  ① sub-agent 병렬 spawn (1단계) — AI 다듬기 ON(기본) 3개 / OFF(ai_polish:false) 0개
       │     (+ type이 auto/비어있으면 meeting-type-classification 1개 추가 — ON에서만)
       │     - speaker-label-correction  → transcript_speaker_edits.json
       │     - speaker-mapping           → speaker_mapping.json
       │     - text-correction           → transcript_text_edits.json
       │     - meeting-type-classification → 파일 미작성, TYPE_DECISION 보고 (type auto일 때만)
       │
       │  ② 모든 sub-agent 종료 후: cp transcript.txt → transcript_corrected.txt
       │  ③ sidecar 적용 — 화자 라벨 + 텍스트 교정 (실패 edit 자동 제외)
       │
       │  ※ AI 다듬기 OFF면 ①~③ 전부 없음 — corrected.txt 미생성, 화자 힌트(notes.json)만
       │    메인이 speaker_mapping.json에 반영 후 phase_step_done correct 전송, 원본으로 ④ 진행
       ▼
transcript_corrected.txt (교정 완성본) + speaker_mapping.json — 이 시점부터 앱이 화자 매핑 편집 허용
       │
       │  ④ 회의 유형 결정 + 회의록 작성 (3·4단계) → meeting-notes.md
       │  ⑤ phase_done (5단계) — 회의록 즉시 공개 (review 화면 전환)
       │  ⑥ 자기검증 (6단계, 검증 ON) — notes-verification 2개 병렬 → 메인 적용 → report
       │     → phase_step_done verify (항상 — 앱 편집 잠금 해제 + 회의록 탭 재로드)
       │     → macOS 알림 (검증 ON/OFF 공통, 항상 마지막)
       ▼
speaker_mapping.json + meeting-notes.md (공개 후 검증 반영) + notes_verification_report.json
       │
       │  ⑦ 앱이 표시 시점에 SPEAKER_XX → 실제 이름 치환 (매핑에 없는 라벨은 "참석자 N" 폴백)
       ▼
사용자에게 실제 이름으로 렌더링된 회의록 표시
```

> **전사 텍스트 교정은 회의록 품질에 기여하지 않음이 실측으로 확정됨** (2026-07-11, 5세션 A/B — 작성자가
> vocabulary·attendees로 자체 교정하므로 오타 유입 0, 내용 정확도 무관). 교정의 가치는 전사본 열람(특히 화자
> 매칭)에 있다. 그럼에도 교정을 1단계에서 **동기로 기다리는** 이유는 단순한 선형 서사(다듬기→작성→검증→완성 —
> 완성된 산출물이 나중에 바뀌지 않음) 때문 — background 분리를 구현했다가 단순성 사유로 회귀함(2026-07-12).
> 비동기 재제안은 유지보수·사용자 이해 비용을 뒤집는 새 근거가 있을 때만.
...
| **그룹핑/메타 표기** (sub-section 헤더, 발표자 표기) | **케이스별 분기** | 아래 별도 항목 참고 |
...
| 영역 | 표기 |
|---|---|
| Sub-section 헤더 (예: 주간 보고의 사람별 sub-section) | 매핑 확정 시 `#### Bobs`, 미확정 시 `#### SPEAKER_06 (백엔드)` |
| 발표자 표기 (위치·유무는 유형 가이드가 정의) | 항상 `- 발표자: SPEAKER_02` (라벨). 발표자 SPEAKER를 특정 못 하면 표기 생략 |

- Sub-section은 미매핑 시 SPEAKER 라벨 박아두면 사용자가 매핑 추가했을 때 자동 치환되어 자연스럽게 채워짐
- 발표자 표기는 매핑 확정 여부와 무관하게 **항상 라벨**: 표시가 항상 현재 매핑을 따르므로 화자 사전 프리필(미확정 추정)이 틀려도 본문에 이름이 박히지 않고, 로컬 AI 경로와 같은 규칙이라 백엔드 간 결과가 일치. `(미확인)` 같은 자리표시는 매핑을 확정해도 스스로 안 바뀌는 노이즈라 금지 ([notes-rules.md](notes-rules.md) "발표자 표기")
...
- ✅ 그룹핑/메타 표기는 케이스별 분기 — sub-section 헤더는 매핑 확정 시 자연어·미확정 시 SPEAKER 라벨, 발표자 표기는 항상 SPEAKER 라벨(특정 못 하면 생략, 위치는 유형 가이드가 정의). 진행자 줄은 작성 X
...
## 1단계: 화자 라벨 교정 + 화자 매핑 + 전사 텍스트 교정 (병렬 sub-agent)

> **이 단계 전체는 내부 분기 판단용 지침입니다.** 아래 등장하는 `AI 다듬기 ON/OFF`·`ai_polish`·파일명(`transcript_corrected.txt` 등)·`sub-agent`·단계 번호는 **사용자 출력에 절대 노출하지 마세요** ([.claude/CLAUDE.md](../../CLAUDE.md) "사용자 친화 출력 규칙"). 이 단계에서 사용자에게 내는 텍스트는 "결과 처리"의 한국어 이모지 요약(`🎤 …`, `📝 …`)뿐이며, 그 전에 "분류가 필요한지 확인합니다" 같은 메타 발화를 출력하면 안 됩니다.

화자 작업·전사 교정 sub-agent를 병렬로 spawn해 transcript.txt를 동시에 분석합니다. 각 작업의 출력 영역이 분리돼 있어 병렬 실행이 안전합니다. **AI 다듬기가 꺼진 회의(`ai_polish: false`)는 이 단계 전체를 건너뜁니다** — 아래 "AI 다듬기 여부 확인" 참고.
...
- **빈 전사 가드 (필수 — 지어내기 방지)**: `transcript_corrected.txt`(있으면) 또는 `transcript.txt`를 Read해 `[SPEAKER_XX M:SS]` 시각 마커를 뺀 **실제 발화 텍스트**를 확인하세요. 발화 내용이 사실상 없으면(마커만 있고 텍스트가 비었거나 공백뿐, 또는 의미 있는 발화가 몇 글자 수준) — 무음 녹음을 사용자가 escape hatch로 강제 진행한 경우 등 — **회의록을 작성하지 말고** 다음만 수행 후 종료하세요. 전사가 비어 있으면 회의 정보(제목·참석자)만으로 가짜 회의록을 지어내게 되므로, 판단이 애매하면 **작성하지 않는 쪽**을 택합니다:
  ...
  2. `$SESSION_DIR/meeting-notes.md`를 Write로 아래 플레이스홀더만 작성 (`{날짜}`는 meeting.json의 date, 참석자 줄은 attendees를 평문으로, 빈 배열이면 `- 참석자: -`):
     ...
     ```
  3. 완료 신호(`app_phase_done` + `app_notify`)를 전송하고 종료 — 이후 단계(sub-agent·회의록 작성·검증) 진행 금지. 사용자 출력은 `⚠️ 인식된 발화가 없어 회의록을 작성하지 않았습니다` 한 줄.
- `$SESSION_DIR/transcript_corrected.txt`가 이미 있으면 이 단계를 건너뛰고 2단계로 진행하세요.

### AI 다듬기 여부 확인 (1단계 진행 여부를 가름)

**`$SESSION_DIR/meeting.json`을 Read**해 `ai_polish` 필드를 확인하세요 (UI 표시명은 "AI 다듬기"). **`false`면 사용자가 시간·토큰 절약을 위해 다듬기를 끈 것**, **그 외(`true` 또는 필드 없음)면 다듬기 포함 — 포함이 기본값**입니다. 이 값이 1단계 진행 여부를 결정하므로 추측하지 말고 반드시 파일을 읽어 판단합니다:

- **다듬기 포함 (기본 — `true` 또는 필드 없음)**: 화자 작업 2개 + `text-correction` = **3개** spawn (+ type이 auto면 분류 1개). 아래 "절차"대로 진행합니다.
- **다듬기 OFF (`false`)**: **sub-agent를 하나도 spawn하지 않고** 아래만 수행한 뒤 곧장 2단계로 넘어갑니다 — corrected.txt·edits JSON을 만들지 않고 `apply-corrections.sh`도 호출하지 않으며, 이후 모든 단계는 원본 `transcript.txt`를 씁니다. **이 전 과정(힌트 매칭 판단 포함)은 말없이 수행하세요** — "AI 다듬기가 꺼진 세션입니다", "화자 힌트를 반영합니다", `t=13(0:13) → SPEAKER_00 = Yotta` 같은 매칭 근거·과정 서술을 채팅에 출력하면 안 됩니다. `ai_polish`·`t`·`SPEAKER_XX`·"탭 시점" 같은 내부 필드·식별자·메커니즘은 사용자 출력 금지([.claude/CLAUDE.md](../../CLAUDE.md) "메타 발화 금지"·"내부 용어 노출 금지"). 사용자에게 보이는 화자 관련 텍스트는 **5단계 요약의 `🎤 화자: …` 한 줄뿐**입니다:
  1. **화자 힌트 반영** — `$SESSION_DIR/notes.json`이 있고 `kind: "speaker"` 항목이 있으면(사용자가 녹음 중 직접 남긴 ground-truth): 각 힌트의 `t`(경과 초)로 transcript.txt에서 해당 시각의 발화 라인을 찾아(라인 헤더 `[SPEAKER_XX M:SS]`의 시각과 대조, ±몇 초 윈도우) 그 SPEAKER 라벨을 힌트의 `speaker` 이름으로 확정하고, **힌트로 확정된 화자만** `$SESSION_DIR/speaker_mapping.json`에 Write하세요 (`{ "SPEAKER_XX": { "name": "이름", "reason": "사용자 화자 힌트 (녹음 중 M:SS 표시)" } }` — reason은 이 형식 그대로, `t=`·매칭 메커니즘을 넣지 말 것. 다른 화자의 엔트리는 만들지 않음. 서로 다른 이름의 힌트가 같은 SPEAKER를 가리키면 그 SPEAKER는 기록하지 않음). **기존 `speaker_mapping.json`이 있으면**(화자분리 단계의 화자 사전 프리필 등) 파일을 읽어 **병합 저장**하세요 — 다른 화자의 기존 엔트리를 유실하지 말 것. 이미 이름이 있는 화자는 덮어쓰지 않되, **예외 하나**: reason이 `지난 회의 목소리 일치`로 시작하는 미확정 프리필은 화자 힌트(ground-truth)가 있으면 힌트를 우선합니다 — **다른 이름**이면 힌트로 교체 + 병기(`"사용자 화자 힌트 (녹음 중 M:SS 표시)\n지난 회의 목소리는 {이름}와 유사였으나 사용자 힌트 우선"`), **같은 이름**이면 이름은 그대로 두고 reason만 힌트를 첫 줄로 승격 + 기존 음성 근거를 둘째 줄에 유지(`"사용자 화자 힌트 (녹음 중 M:SS 표시)\n지난 회의 목소리 일치: …"`) — 같은 결론이어도 사용자 직접 입력이 더 강한 근거인데 음성 유사도만 보이면 안 됩니다. 힌트가 없으면 매핑 파일을 새로 만들지 않습니다(기존 프리필 파일은 그대로 둠) — 앱이 표시 시점에 "참석자 N"으로 폴백합니다.
  2. **AI 다듬기 완료 신호 전송** (필수 — 누락하면 앱이 다듬기 단계에 고착되어 화자 편집 잠금이 안 풀립니다. 재작성 모드가 1단계를 skip할 때도 신호는 보내는 것과 같은 규칙):
     ```bash
     bash -c 'source "$APP_DIR/lib/signal.sh" && app_phase_step_done correct'
     ```
  3. TodoWrite의 "AI 다듬기" 항목을 `completed`로 마킹. 이모지 요약 블록(🎤·📝)은 내지 않습니다 — 다듬기를 하지 않았으므로 보고할 결과가 없습니다.
  4. 2단계로 진행 — 단 2단계의 "화자 매칭 및 회의 요약 출력"에서 화자 매칭 블록은 생략하고 `📋 회의 요약`만 출력, "AI 다듬기 완료 신호"는 이미 보냈으므로 다시 보내지 않습니다. type이 `auto`면 분류 sub-agent가 없으므로 3단계에서 인라인 자동 판단합니다.

다듬기 OFF는 화자 라벨 교정·이름 매칭·전사 텍스트 교정이 모두 빠지므로 **회의록의 발화 귀속 품질이 하락**할 수 있습니다(사용자가 안내받고 선택한 트레이드오프). 4단계의 vocab·attendees 자체 교정이 그만큼 더 중요해집니다.
...
- **`auto` 또는 비어있음** → 분류 sub-agent **추가** (foreground)
...
AI 다듬기 OFF면 이 분류 sub-agent도 spawn하지 않습니다 (1단계 전체 skip) — `auto`여도 3단계의 인라인 자동 판단이 처리합니다.
...
교정된 전사본(`$SESSION_DIR/transcript_corrected.txt` — **없으면(AI 다듬기 OFF) 원본 `transcript.txt`**)과 1단계 sub-agent 결과를 활용해 회의 내용을 파악합니다.
...
**AI 다듬기 OFF면 화자 매칭 블록은 생략**하고 `📋 회의 요약`만 출력합니다 — 매칭 작업을 하지 않았으므로 보고할 추정이 없습니다 (화자 힌트로 확정한 이름이 있어도 사용자가 직접 남긴 정보라 재보고 불필요).
...
여기까지가 "AI 다듬기" 단계 — 사이드바 stepper에서 ✓ 표시되어 회의록 작성으로 전환됩니다. **AI 다듬기 OFF면 1단계에서 이미 보냈으므로 다시 보내지 않습니다**:
```bash
bash -c 'source "$APP_DIR/lib/signal.sh" && app_phase_step_done correct'
```
...
1. `$SESSION_DIR/meeting.json`의 `type` 필드가 명시 templates 유형이면 → `~/Library/Application Support/app.junmit/templates/{type}.md`를 Read. **그 파일이 없으면**(사용자가 해당 유형을 삭제했거나 이름이 바뀐 경우) → 1단계에서 분류 sub-agent를 안 띄웠으므로 여기서 인라인 자동 판단(`notes-rules.md` "자동 판단" 기준 — 제목 × `title_keywords` 0순위 포함): 위 디렉토리의 모든 `*.md`의 frontmatter(`title_keywords`·`summary`)를 수집해 회의 제목·내용(전사본 — `transcript_corrected.txt`, 없으면 `transcript.txt` — + `meeting.json`의 `agenda`)과 매칭해 결정하고 `meeting.json.type` 갱신. 명시 유형이 사라졌다고 작성을 멈추지 말 것
2. `type`이 `free-form`이면 → 자동 판단 skip + `notes-rules.md`의 "Free-form 작성" 절차 직접 적용
3. `type`이 `auto` 또는 비어있으면 →
   - **1단계에서 분류 sub-agent가 돈 경우**(AI 다듬기 ON 첫 작성): `meeting-type-classification` sub-agent가 이미 결정했으므로 그 보고의 `TYPE_DECISION:` 값(유형 id 또는 `free-form`)을 채택해 **`meeting.json.type`에 즉시 갱신** (재분류하지 말 것 — 이미 병렬로 끝남).
   - **1단계를 skip한 경우**(AI 다듬기 OFF, 또는 `transcript_corrected.txt`가 이미 존재해 sub-agent를 안 띄움 — 재작성 모드·크래시 후 재시도): 분류 sub-agent가 돌지 않아 `TYPE_DECISION` 보고가 **없습니다**. 이때는 여기서 **인라인 자동 판단**: 위 디렉토리의 모든 `*.md`의 frontmatter(`title_keywords`·`summary`)를 수집해 회의 제목·내용(전사본 — `transcript_corrected.txt`, 없으면 `transcript.txt` — + `meeting.json`의 `agenda`)과 매칭(`notes-rules.md` "자동 판단" 기준 — 제목 × `title_keywords` 0순위 포함) → 결정해 **`meeting.json.type`에 갱신**.
...
1. 요약 출력 (사용자 정보용) — **반드시 2번 신호보다 먼저** 출력합니다. 신호를 받은 앱은 곧장 다음 처리(검증 시작 또는 완료 전환)로 넘어가므로, 요약이 신호 뒤에 나오면 이미 끝난 단계의 결과가 진행 표시에 뒤늦게 나타나는 어색한 순서가 됩니다:
   - **첫 작성**:
     ```
     ✅ 회의록 초안 작성 완료
   ...
   - **재작성 모드** (1단계 sub-agent skip이라 N·K 값 없음 — 단순 출력, 유형은 위와 같이 표시 이름으로):
     ```
     ✅ 회의록 초안 재작성 완료 — 유형: {유형 표시 이름} ({세션/논의 수}개 주제)
     ```
   - **AI 다듬기 OFF 첫 작성**도 화자 매칭 N·K 값이 없으므로 같은 단순 형식으로 출력하되, 녹음 중 화자 힌트가 있었으면(1단계에서 speaker_mapping.json에 반영) 그 결과를 **깨끗한 한 줄로만** 덧붙입니다 (내부 필드·매칭 과정 서술 금지 — 이 줄이 유일한 화자 표기):
     ```
     ✅ 회의록 작성 완료 — 유형: {유형 표시 이름} ({세션/논의 수}개 주제)
     🎤 화자: 녹음 중 힌트로 {이름1}·{이름2} 확정. 나머지는 미확인이에요. 회의록 탭에서 확인·매칭하면 이름이 채워져요
     ```
     힌트가 없었으면 `🎤` 줄은 생략하고 `✅`만 출력합니다. ("초안" 여부는 아래 검증 규칙을 따름 — OFF+검증 OFF 조합이면 "작성 완료".)
   - **"초안"은 검증이 이어질 때만** — `meeting.json`의 `notes_verification`이 `false`(검증 OFF)면 이 시점이 최종본이므로 "초안"을 빼고 `✅ 회의록 작성 완료` / `✅ 회의록 재작성 완료 — …`로 출력합니다. 검증 ON(기본)에선 6단계가 본문을 다듬을 수 있으므로 "작성 완료"가 완성본처럼 읽히지 않게 "초안"을 붙입니다. 값을 아직 안 읽었다면(재작성 모드는 1단계의 meeting.json 확인을 건너뜀) 추측하지 말고 이 시점에 `meeting.json`을 확인하세요.

2. **회의록 작성 완료 신호 전송** — 앱이 Activity.Idle 전환 + 회의록 공개(탭은 열람 가능). 검증이 이어지는 경우 회의록 탭 자동 이동·완료 표기·사이드바 "AI에게 추가 요청"(에이전트 CLI 한정) 노출·알림은 앱이 6단계 verify 신호 시점에 처리(그동안 사용자는 화자 매핑을 계속. 검증 중 추가 요청을 허용하면 headless에선 검증 프로세스와 resume 스폰이 같은 세션 파일을 동시에 쓰게 되어 앱이 노출을 미룬다). 회의록 내보내기는 회의록 탭의 "복사" 버튼:
   ```bash
...
회의록을 공개한 뒤, 전사와 대조해 사후 검증하고 반영합니다. 공개된 초안이 검증으로 소폭 바뀔 수 있으나(사용자가 이미 볼 수 있음), 바뀐 내역은 "검증 N건" 칩으로 투명하게 노출되므로 문제없습니다. **검증이 도는 동안 앱은 회의록 본문 편집·유형 변경을 잠급니다**(동시 수정 충돌 방지) — 잠금은 아래 4번 신호로 풀리므로, 검증을 시작했다면 결과와 무관하게 4번 신호를 반드시 보내세요.
...
3. 두 보고를 받으면 **메인이 지적들을 `meeting-notes.md`에 직접 적용** (Edit — 보고의 원문/수정 쌍을 기계적으로 반영. 두 보고가 같은 문장을 다르게 고치라면 라인 근거가 더 구체적인 쪽 채택). **Edit의 원문 매칭이 실패한 항목은 건너뛰고 report에도 넣지 않습니다** — 본문 재작성·전체 갱신으로 우회하지 마세요 (그 문장이 이미 바뀌었다면 지적의 전제도 무효). 적용이 1건 이상이면 내역을 `$SESSION_DIR/notes_verification_report.json`에 Write — 앱이 회의록 탭에 "검증 N건" 칩으로 표시하는 원천:
   ...
   {
     "applied": [
       { "type": "근거없음|누락|귀속|정도|병합", "before": "수정 전 서술 (누락이면 빈 문자열)", "after": "수정 후 서술 (또는 추가 요지)", "evidence": "L372-373", "note": "한 줄 사유 (한국어, 사용자가 읽음)" }
     ]
   }
   ```
   매핑 의심(⚠) 보고가 있으면 `"mapping_warnings": ["..."]` 배열로 함께 기록 (회의록 수정은 아니지만 화자 매칭 화면에서 참고할 정보). **적용 0건이라도 매핑 의심이 있으면 `applied: []` + `mapping_warnings`로 파일을 씁니다** (화자 매칭에 중요한 신호라 앱 칩으로 노출). 적용 0건 + 매핑 의심도 없으면 파일을 만들지 않습니다.
4. **검증 결과 출력 → 검증 종료 신호** — 먼저 검증 결과 한 줄을 출력하고(0건이면 생략, diff는 출력하지 않음): `📋 검증 {V}건 반영`, **바로 이어서** 신호를 보냅니다. 요약이 신호보다 먼저여야 앱이 완료 전환(회의록 탭 이동·완료 표기)을 한 뒤에 결과 줄이 뒤늦게 나타나지 않습니다.
   **신호는 필수 — 결과 출력 여부·report 유무·적용 0건 무관, 검증을 수행했다면 항상.** 앱이 이 신호로 편집 잠금을 풀고 회의록 탭(본문 + 검증 칩)만 다시 읽습니다. 전사본 탭은 검증이 건드리지 않으므로 전체 갱신(`app_refresh`)이 아니라 이 신호를 씁니다. 누락하면 편집 잠금이 타임아웃(10분)까지 풀리지 않습니다:
   ```bash
   bash -c 'source "$APP_DIR/lib/signal.sh" && app_phase_step_done verify'
   ```
   - **sub-agent가 실패하거나 비정상 종료해도 중단하지 않습니다** — 살아있는 쪽 보고만 적용하고(둘 다 실패면 초안 그대로) 4번 신호를 보낸 뒤 5번으로 진행 (검증은 안전망이지 관문이 아님).
5. macOS 알림 전송 (사용자가 다른 앱에 있을 수 있으므로 — 검증 ON/OFF 공통, 항상 마지막에. 모든 단계가 끝난 뒤에 보내야 알림으로 복귀한 사용자가 완성본을 봅니다):
   ```bash
   bash -c 'source "$APP_DIR/lib/signal.sh" && app_notify "회의록이 준비되었습니다. 확인해주세요."'
   ```

재작성 모드에서도 검증은 동일하게 수행합니다.

마지막으로 TodoWrite의 "마무리" 항목을 `completed`로 마킹하고 안내 메시지를 출력합니다:
```
✅ 회의록 준비 완료. 회의록 탭에서 결과를 검토해주세요.
...
**시작 전 사전 로드 (AI 다듬기 OFF에서 특히 중요):** `~/Library/Application Support/app.junmit/vocabulary.json`의 `terms` 배열을 Read하세요. 사용자가 등록한 기술·도구·도메인 용어로, 회의록을 쓰면서 음성 오인식을 자체 교정하는 핵심 입력입니다. **AI 다듬기 OFF에선 별도 전사 텍스트 교정(text-correction) 단계가 없으므로**, 회의록 작성자인 당신이 vocabulary·참석자 이름(meeting.json의 attendees)·문맥으로 직접 교정하며 본문을 작성해야 합니다. (ON에선 1단계가 `transcript_corrected.txt`를 이미 교정했지만, vocab 프리로드는 무해하며 본문 작성 시 추가 안전망입니다.)
