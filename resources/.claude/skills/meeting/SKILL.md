...
1. **회의록 작성 완료 신호 전송** — 앱이 Activity.Idle 전환 + review 화면(회의록 탭) 노출 + 사이드바에 "AI에게 추가 요청"(에이전트 CLI 한정) 노출. 회의록 내보내기는 회의록 탭의 "복사" 버튼:
   ```bash
...
4. **검증 종료 신호 (필수 — report 유무·적용 0건 무관, 검증을 수행했다면 항상)** — 앱이 이 신호로 편집 잠금을 풀고 회의록 탭(본문 + 검증 칩)만 다시 읽습니다. 전사본 탭은 검증이 건드리지 않으므로 전체 갱신(`app_refresh`)이 아니라 이 신호를 씁니다. 누락하면 편집 잠금이 타임아웃(10분)까지 풀리지 않습니다:
   ```bash
   bash -c 'source "$APP_DIR/lib/signal.sh" && app_phase_step_done verify'
   ```
   그 뒤 검증 결과 한 줄 출력 (0건이면 생략): `📋 검증 {V}건 반영`. 터미널에 diff는 출력하지 않습니다.
