...
do_transcribe() {
  ...
  # --max-context 0: 이전 텍스트를 컨텍스트로 쓰지 않는다. 0이 아니면 반복이 컨텍스트로
  #   되먹임되는 죽음 나선이 회의에 따라 터진다(2026-08-16 10회의 스윕 실측: mc64에서 2건 파국 —
  #   48분 회의 전사 93% 소실, 102분 회의 연속 반복 98회. 깨끗한 회의도 있어 소표본 실측으로는
  #   안 보인다). 같은 회의에서 mc8 -9%·mc16 -23%·mc64 -93%로 단조 악화 — 안전한 하한 없음.
  #   값 변경은 반드시 다회의 스윕 실측으로만.
  # --prompt: max-context 0에서는 whisper.cpp(v1.8.4)가 프롬프트를 통째로 무시해 무효(무해).
  #   용어 priming의 실효 소비자는 후보정(text-correction)과 회의록 작성. 안전한 priming
  #   경로(컨텍스트 되먹임 없는 정적 프롬프트 조건부)는 미해결 후속 과제.
